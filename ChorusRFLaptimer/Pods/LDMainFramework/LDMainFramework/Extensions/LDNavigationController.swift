...
extension UINavigationController
{
    ...
    open func ldGoBack(animated:Bool = true)
    {
    ...
    open func ldPopToRootVC(animated:Bool = true)
    {
    ...
    open func ldOpenVC(_ vc:UIViewController, animated:Bool = true, clearAll:Bool = false, popToVCIfExists:Bool = false, allowSameVC:Bool = false)
    {
    ...
    open func ldClearAllPreviousPages()
    {
    ...
    ///   - allowSameVC: Defines if it is allowed to open a view controller if a view controller wish same restorationIdentifier is already on the top of the navigation stack. Default value is false.
    open func ldOpenVC(_ identifier:String, storyBoardName:String = "Main", animated:Bool = true, clearAll:Bool = false, popToVCIfExists:Bool = false, allowSameVC:Bool = false)
    {
        ldOpenVC(ldGetVCForIdentifier(identifier, storyBoardName: storyBoardName), animated:animated, clearAll:clearAll, popToVCIfExists:popToVCIfExists, allowSameVC:allowSameVC)
    }
    ...
    open func ldGetVCForIdentifier(_ identifier:String, storyBoardName:String = "Main") -> UIViewController
    {
    ...
    /// Helper method which is used for getting top view controller in application. Top view controller is used for accessing navigation controller for ldGetNC() method
    ///
    ...
    static open func ldTopVC(controller: UIViewController? = UIApplication.shared.keyWindow?.rootViewController) -> UIViewController? {
        ...
        if let tabController = controller as? UITabBarController {
            if let selected = tabController.selectedViewController {
                return ldTopVC(controller: selected)
            }
        }
        if let presented = controller?.presentedViewController {
            return ldTopVC(controller: presented)
        }
        return controller
    }
